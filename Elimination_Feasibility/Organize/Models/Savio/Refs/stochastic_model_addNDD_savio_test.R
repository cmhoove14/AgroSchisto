...
#First run not in for loop
fill.arr[1, , ] = stoch.sim.noise(init = stoch_sims_eqbm_vals_fin[1,c(3:7)], 
                                  lam = stoch_sims_eqbm_vals_fin[1,2], 
                                  k = stoch_sims_eqbm_vals_fin[1,1],
                                  sim = 1)[c(1,2,4,5,6)]
  
#First run not in parallel
for(s in c(min.sim:max.sim)){

  fill.arr[s, , ] = 
  sapply(c(1:stoch.sims), stoch.sim.noise, init = stoch_sims_eqbm_vals_fin[s,c(3:7)], 
