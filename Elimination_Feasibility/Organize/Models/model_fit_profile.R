...
Tx_simulate<-function(par_triplet){
  params["beta"]<-par_triplet[1]
  params["lamda1"]<-par_triplet[2]
  params["lamda2"]<-par_triplet[3]
  params["lamda"]<-( (375/594) * par_triplet[2]) +( (219/594) * par_triplet[3]) 
...
#test
Tx_simulate(c(betas[5], lamda1s[5], lamda2s[5]))

#Fit using Optim
op_min <- optim(par=c(betas[5], lamda1s[5], lamda2s[5]), Tx_simulate, method="Nelder-Mead")

#Extract best fitting parameters
  bestBeta<-op_min$par[1]
  bestLamda1<-op_min$par[2]
  bestLamda2<-op_min$par[3]
...
#Now do the grid search on a 3D grid of beta, lamda1 and lamda2 around the best fit parameter values ########
  beta_range<-seq(from=bestBeta*0.1, to=bestBeta*1.9, length.out = 10)
...
par_grid <- expand.grid(beta_range, lamda1_range, lamda2_range)  
...
fin_pars <- cbind(par_grid, negLL)

boundary <- Tx_simulate(bestBeta,bestLamda1,bestLamda2) + 7.815 #chi-square with 3 degree of freedom, 95% CI
