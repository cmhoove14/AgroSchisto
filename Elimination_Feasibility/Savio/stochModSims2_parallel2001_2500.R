...
#run all simulations######
for(s in min.sim:max.sim){
  s2 = s - min.sim + 1

#Run sims for parameter set  
  fill.arr[s2, , ] =
  parSapply(clust, c(1:stoch.sims), stoch.sim, init = par.mat[s2,c(3:7)], 
                                                lam = par.mat[s2,1], 
...
arr.name = paste('~/ElimFeas_StochMod/fill_array', min.sim, '_', max.sim, '.Rdata', sep = '')
