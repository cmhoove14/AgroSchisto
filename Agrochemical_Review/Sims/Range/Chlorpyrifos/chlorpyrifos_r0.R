...
#Chlorpyrifos response functions for S. mansoni
rfx_chlorpyrifos <- rfx_sum %>% filter(Chemical == "Chlorpyrifos" & 
                                     best == 1) %>% 
  rename(Parameter = parameter,
         Study = study_long,
         study_abrev = Study)

#Load relevant Chlorpyrifos response functions ######
  chlorpyrifos_studies <- unique(rfx_chlorpyrifos$study_abrev)
  chlorpyrifos_functions <- rfx_chlorpyrifos$rfx
  chlorpyrifos_files <- rfx_files[unlist(sapply(chlorpyrifos_studies, grep, rfx_files, ignore.case = TRUE))]

sapply(paste0(rfx_dir, chlorpyrifos_files), source)
...
r0_chlor <- function(chlor){
  r0_chlors <- parSapply(cl = clust, 
                       rep(chlor, nsims), 
                       r0.In,
                       f.f_Nq = fNq_chlor_ibr92_uncertainty, 
...
save(chlor_sims, file = "Agrochemical_Review/Sims/Range/chlorpyrifos/chlor_r0_sims.RData")
