  ...
  eec_sims %>% 
    ggplot(aes(y = R0, x = Study_ID_Unique, fill = Pathway)) + 
      geom_rect(ymin = base_r0_q025, ymax = base_r0_q975,
                xmin = 0, xmax = nrow(eec_sims), alpha = 0.2, fill = "grey80") +
...
#Ggplot classic forestplot figure  #######
  eec_sims %>% 
    ...
    summarise(med_R0 = median(R0),
              ...
              Class = first(Class)) %>% 
    ggplot(aes(y = Study_ID_Unique, x = med_R0, xmin = R0_25, xmax = R0_75, col = Pathway)) + 
      geom_rect(xmin = base_r0_q25, xmax = base_r0_q75,
                ymin = 0, ymax = nrow(eec_sims), alpha = 0.2, fill = "grey80") +
