...
ggsave("../Plots/obs_chems_distributions.jpg",
...
gg_forest_eec <- gg_forest_df %>%
  ...
  filter(sig == 1) %>% 
    ggplot(aes(y = as.factor(order), 
      ...
      labs(x = expression(paste(italic(R['0jc']), 
                                "(", italic(EEC[c]), ")", 
                                sep = "")))
  ...
  #Put them all together and plot  
    range_plot <- bind_rows(atr_conc_range_fin,
      ...
      ggplot(aes(x = EEC_conc, 
        ...
        labs(y = expression(paste(italic(R['0c']), "(", italic(q[c]), ")")),
...
### All DALYs Sims  
```{r all_sims_through_time, message = F, cache = T, cache.lazy = FALSE}
require(parallel)
cl <- makeCluster(detectCores()-1)
clusterExport(cl, ls())
...
rm(list = setdiff(ls(), c("fit_pars", "agrochem_time_series", "theme_ms", "years", "area",
...
atr_annual_ts <- get_ts_from_sim(atr_annual_simulation, 1)
  #comp_ts_across_sims(atr_annual_simulation)

chlor_annual_ts <- get_ts_from_sim(chlor_annual_simulation, 1)
  #comp_ts_across_sims(chlor_annual_simulation)

gly_annual_ts <- get_ts_from_sim(gly_annual_simulation,1)
  #comp_ts_across_sims(gly_annual_simulation)

mal_annual_ts <- get_ts_from_sim(mal_annual_simulation, 1)
  #comp_ts_across_sims(mal_annual_simulation)

prof_annual_ts <- get_ts_from_sim(prof_annual_simulation,1)
  #comp_ts_across_sims(prof_annual_simulation)
```
...
saveRDS(comp_dalys_all_annual, "comp_dalys_all_annual.rds")
...
```
##### Snail population through time  
```{r atr_ts_snails}
#Do some cleaning to atr time series data for plotting snail dynamics  
atr_annual_ts_snails <- atr_annual_ts %>% 
  filter(Variable %in% c("S", "E", "I") & 
           sim_run == 1 & 
           sim_type %in% c("Agro_Preds_MDA", "MDA_Preds")) %>% 
  spread(Variable, Value) %>% 
  mutate(N = S+E+I,
         Scenario = factor(sim_type, 
                           levels = c("MDA_Preds",
                                      "Agro_Preds_MDA"),
                           labels = c("MDA, Predators",
                                      "MDA, Predators, Agrochemical")))

#Remove sampling variability from response function from plot
atr_annual_ts_snails$N[which(atr_annual_ts_snails$time < first_atr & 
                               atr_annual_ts_snails$Scenario == "MDA, Predators, Agrochemical")] <- 
  atr_annual_ts_snails$N[which(atr_annual_ts_snails$time < first_atr & 
                               atr_annual_ts_snails$Scenario == "MDA, Predators")]

#Show dynamics of mean worm burden through time ############
atr_snail_dynamics <- ggplot() +
    geom_rect(data = atr_conc_ts,
              aes(ymin = -Inf,
                  ymax = Inf,
                  xmin = day, xmax = day +1,
              ...
              alpha = 0.7) +
    scale_fill_gradient(low = "white", high = "#FDAF91FF") +
    geom_line(data = atr_annual_ts_snails, 
              aes(x = time, 
                  y = N/area, 
                  col = Scenario), 
              size = 1) +
    scale_color_manual(values = c("violet", "purple")) +
    labs(y = expression("Total Snail Density"),
         x = "Time (years)") +
    scale_x_continuous(breaks = 365*c(0:11),
                       labels = c(-1:10)) +
    theme_ms() +
...
ggsave("../Plots/atrazine_ts_snail_dynamics.jpg", atr_snail_dynamics,
       width = 8, height = 6, dpi = 300, units = "in")
```
...
comp_dalys_atr_df <- comp_dalys_atr_annual %>% 
  ...
  mutate(scenario_type = case_when(Scenario == "Nothing" ~ "base",
         ...
         Scenario = factor(Scenario, 
                           ...
                           labels = c("No Intervention",
                                      ...
                                      "MDA, Predators",
                                      "MDA, Predators, \nAgrochemical")))
comp_dalys_atr <- comp_dalys_atr_df %>% 
  ggplot(aes(x = scenario_type, y = (DALYs/fit_pars["H"]/years)*100000, fill = Scenario)) +
    geom_boxplot() +
    scale_fill_manual(values = c("red", "darkred",
                                 "cyan2", "darkblue",
                                 "violet", "purple")) +
    scale_y_continuous(position = "right",  limits = c(0,1250)) +
...
atr_dalys_plot <- ggplotGrob(comp_dalys_atr +
                               annotate("text", x = 3.2, y = 1200, size = 8,
...
##### Snail and predator dynamics through time  
```{r chlor_snails_tx}
#Do some cleaning to chlor time series data for plotting snail dynamics  
chlor_annual_ts_snails <- chlor_annual_ts %>% 
  filter(Variable %in% c("S", "E", "I", "P") & 
           sim_run == 1 & 
           sim_type %in% c("Agro_Preds_MDA", "MDA_Preds")) %>% 
  spread(Variable, Value) %>% 
  mutate(N = S+E+I,
...
#Show dynamics of snail dnsity and predator density through time ############
chlor_snail_dynamics <- ggplot() +
    geom_rect(data = chlor_conc_ts,
              aes(ymin = -Inf,
                  ymax = Inf,
                  xmin = day, xmax = day +1,
              ...
              alpha = 0.7) +
    scale_fill_gradient(low = "white", high = "#925E9F99") +
    geom_line(data = chlor_annual_ts_snails, 
              aes(x = time, 
                  y = Value/area,
                  col = Scenario),size = 1) +
    scale_color_manual(values = c("violet", "purple")) +
...
ggsave("../Plots/chlorpyrifos_ts_snail_dynamics.jpg", chlor_snail_dynamics,
       width = 8, height = 6, dpi = 300, units = "in")

```

##### DALYs accumulation through time  
```{r chlor_dalys_box}
#Show compariosn of DALYs accumulated in different intervention scenarios #########
comp_dalys_chlor_df <- comp_dalys_chlor_annual %>% 
  gather("Scenario", "DALYs", Nothing:Agro_Preds_MDA) %>% 
  filter(Application == "Annual" & 
           Scenario %in% c("Nothing", "Agro_only",
                           "MDA_only", "Agro_MDA",
...
comp_dalys_chlor <- comp_dalys_chlor_df %>% 
  ggplot(aes(x = scenario_type, y = (DALYs/fit_pars["H"]/years)*100000, fill = Scenario)) +
    geom_boxplot() +
    scale_fill_manual(values = c("red", "darkred",
                                 "cyan2", "darkblue",
...
#### Glyphosate  
##### Worm burden dynamics through time  
```{r gly_ts_plot}
...
##### Snail population dynamics through time  
```{r gly_snail_ts}
#Do some cleaning to glyphosate time series data for plotting snail dynamics  
gly_annual_ts_snails <- gly_annual_ts %>% 
  filter(Variable %in% c("S", "E", "I") & 
           sim_run == 1 & 
           sim_type %in% c("Agro_Preds_MDA", "MDA_Preds")) %>% 
  spread(Variable, Value) %>% 
...
#Remove sampling variability from response function from plot
gly_annual_ts_snails$N[which(gly_annual_ts_snails$time < first_gly & 
                               gly_annual_ts_snails$Scenario == "MDA, Predators, Agrochemical")] <- 
  gly_annual_ts_snails$N[which(gly_annual_ts_snails$time < first_gly & 
                               gly_annual_ts_snails$Scenario == "MDA, Predators")]

#Show dynamics of mean worm burden through time ############
gly_snail_dynamics <- ggplot() +
    geom_rect(data = gly_conc_ts,
              aes(ymin = -Inf,
                  ymax = Inf,
                  xmin = day, xmax = day +1,
              ...
              alpha = 0.7) +
    scale_fill_gradient(low = "white", high = "#42B54099") +
    geom_line(data = gly_annual_ts_snails, 
              aes(x = time, 
                  y = N/area, 
                  col = Scenario), 
              size = 1) +
...
ggsave("../Plots/glyphosae_ts_snail_dynamics.jpg", gly_snail_dynamics,
       width = 8, height = 6, dpi = 300, units = "in")
...
```{r gly_DALYs}
#Show compariosn of DALYs accumulated in different intervention scenarios #########
comp_dalys_gly_df <- comp_dalys_gly_annual %>% 
  gather("Scenario", "DALYs", Nothing:MDA_Preds_attributable) %>% 
  filter(Application == "Annual" & 
           Scenario %in% c("Nothing", "Agro_only",
                           "MDA_only", "Agro_MDA",
  ...
  mutate(scenario_type = case_when(Scenario == "Nothing" ~ "base",
         ...
         Scenario = factor(Scenario, 
                           ...
                           labels = c("No Intervention",
                                      ...
                                      "MDA, Predators",
                                      "MDA, Predators, \nAgrochemical"))) 
comp_dalys_gly <- comp_dalys_gly_df %>% 
  ggplot(aes(x = scenario_type, y = (DALYs/fit_pars["H"]/years)*100000, fill = Scenario)) +
    geom_boxplot() +
    scale_fill_manual(values = c("red", "darkred",
                                 "cyan2", "darkblue",
...
##### Snail and Predator dynamics through time  
```{r prof_snail_pred_ts}
#Do some cleaning to prof time series data for plotting snail dynamics  
prof_annual_ts_snails <- prof_annual_ts %>% 
  filter(Variable %in% c("S", "E", "I", "P") & 
           sim_run == 1 & 
           sim_type %in% c("Agro_Preds_MDA", "MDA_Preds")) %>% 
  spread(Variable, Value) %>% 
...
prof_snail_dynamics <- ggplot() +
    geom_rect(data = prof_conc_ts,
              ...
              alpha = 0.7) +
    scale_fill_gradient(low = "white", high = "#0099B499") +
    geom_line(data = prof_annual_ts_snails, 
              aes(x = time, 
                  y = Value/area, 
                  col = Scenario), 
              size = 1) +
...
ggsave("../Plots/pofenofos_ts_snail_dynamics.jpg", prof_snail_dynamics,
       width = 8, height = 6, dpi = 300, units = "in")
...
```{r prof_DALYs}
#Show compariosn of DALYs accumulated in different intervention scenarios #########
comp_dalys_prof_df <- comp_dalys_prof_annual %>% 
  gather("Scenario", "DALYs", Nothing:MDA_Preds_attributable) %>% 
  filter(Application == "Annual" & 
           Scenario %in% c("Nothing", "Agro_only",
                           "MDA_only", "Agro_MDA",
  ...
  mutate(scenario_type = case_when(Scenario == "Nothing" ~ "base",
         ...
         Scenario = factor(Scenario, 
                           ...
                           labels = c("No Intervention",
                                      ...
                                      "MDA, Predators",
                                      "MDA, Predators, \nAgrochemical")))
comp_dalys_prof <- comp_dalys_prof_df %>% 
  ggplot(aes(x = scenario_type, y = (DALYs/fit_pars["H"]/years)*100000, fill = Scenario)) +
    geom_boxplot() +
    scale_fill_manual(values = c("red", "darkred",
                                 "cyan2", "darkblue",
...
all_grob <- arrangeGrob(atr_grob, chlor_grob, 
                        gly_grob, prof_grob,
                        nrow = 4, ncol = 1,
                        left = "Mean parasite burden (W)",
                        right = "Annual DALYs lost per 100,000 people")

ggsave("../Plots/all_ts_plots2.jpg", all_grob,
       width = 8, height = 11.5, dpi = 300, units = "in")
```

### Comparison to other conditions in GBD 2017  
...
chem_risk_factors <- comp_dalys_all_annual %>% 
  gather("Scenario", "DALYs_p100k", Agro_att_p100k:MDA_Preds_att_p100k) %>% 
  group_by(Scenario, Chemical) %>% 
  filter(Scenario != "Pred_att_p100k") %>% 
  summarise(Value = median(DALYs_p100k),
            lower = quantile(DALYs_p100k, 0.025),
            upper = quantile(DALYs_p100k, 0.975)) %>% 
  mutate(`Risk factor` = case_when(Scenario == "Agro_att_p100k" ~ paste(Chemical, " pollution only"),
                                      Scenario == "MDA_att_p100k" ~ paste(Chemical, " pollution + MDA"),
                                      Scenario == "MDA_Preds_att_p100k" ~ paste(Chemical, " pollution + MDA + Preds"))) %>% 
  ungroup() %>% 
  dplyr::select(`Risk factor`, Value, lower, upper) %>% 
  filter(Value > 1)

rbind(sen_dalys2017_risk_factors %>% 
        filter(Value < max(comp_dalys_all_annual$MDA_Preds_att_p100k) & 
                 Value > 5) %>% 
        mutate(Chem = "No"),
      chem_risk_factors %>% 
        mutate(Chem = "Yes")) %>% 
  ggplot(aes(x = `Risk factor`, y = Value, ymin = lower, ymax = upper, col = Chem)) + 
    geom_point() +
    #scale_color_discrete(values = c("black", "red")) +
    geom_errorbar(width = 0.2) +
    theme_classic() +
    ...
    ylim(c(0,300)) +
    labs(y = "DALYs lost per 100,000 per year",
         title = "Comparable risk factors in GBD 2017 study")

ggsave("../Plots/GBD_2017_comparable_risk_factors.jpg",
       width = 8, height = 6, dpi = 300, units = "in")
```
...
```{r scrap, eval=FALSE, include=FALSE}
  ...
  mutate(`Risk Factor` = factor(`Risk Factor`, 
                                levels = c(chem_risk_factors %>% 
                                             arrange(Value) %>% 
                                             pull(`Risk factor`),
                                           sen_dalys2017_risk_factors %>% 
                                             arrange(Value) %>% 
