...
![Forestplot of agrochemical effects with median effect outside of IQR of baseline $R_0$ estimates at peak EEC](../Plots/Forestplot_sig_effects_eec.jpg)
...
```{r all_peak_obs_foretplot, warning = FALSE}
  ...
  #Put them all together and plot  
    range_plot_peak_obs <- bind_rows(atr_peak_obs_conc_range_fin,
      ...
      ggplot(aes(x = rel_conc, 
        ...
        geom_ribbon(alpha = 0.25, size = 0.01) +
        geom_hline(yintercept = base_r0_median, lty = 2) +
        ylab(expression(italic(R[0](q[c])))) + 
        #ggtitle(expression(paste("Combined effects of key agrochemicals on estimates of ", R[0], "(q)"))) +
        scale_x_continuous(trans = "log",
                           breaks = c(0.00001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1),
                           limits = c(0.00001,1.1)) +
...
rm(list = setdiff(ls(), c("atr_mw2013_ts", 
                          "chlor_ca_surf_ts", 
                          "gly_ca_surf_ts", 
                          "mal_ca_surf_ts")))
...
rm(list = setdiff(ls(), c("atr_mw2013_ts", 
                          ...
                          "mal_ca_surf_ts",
                          "mal_annual_simulation", "mal_biannual_simulation")))
...
added_dalys <- 
  comp_dalys_all %>% filter(Application == "Annual") %>% 
...
atr_annual_ts_clean <- atr_annual_ts %>% 
  filter(Variable == "W" & 
           sim_run == 1 & 
...
atr_dynamics <- ggplot() +
    ...
    geom_line(data = atr_annual_ts_clean, 
              aes(x = time, 
                  y = Value, 
                  col = Scenario), 
              size = 1) +
...
comp_dalys_atr <- comp_dalys_atr_annual %>% 
  ...
  ggplot(aes(x = scenario_type, y = DALYs, fill = Scenario)) +
    ...
    theme(axis.ticks.x = element_blank(),
          legend.title = element_text(9),
          legend.text = element_text(7),
...
chlor_annual_ts_clean <- chlor_annual_ts %>% 
  filter(Variable == "W" & 
           sim_run == 1 & 
           sim_type %in% c("Nothing", "Agro_only", 
                           "MDA_only", "Agro_MDA",
...
chlor_dynamics <- ggplot() +
    ...
    geom_line(data = chlor_annual_ts_clean, 
              aes(x = time, 
                  y = Value, 
                  col = Scenario), 
              size = 1) +
