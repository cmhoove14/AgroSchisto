...
```{r source_par_set_generation, cache = T}
source("../Sims/EEC/generate_eec_parameter_sets.R")
source("../Sims/EEC/generate_peak_obs_parameter_sets.R")
```
...
gg_forest_df <-  eec_sims %>% 
    ...
    summarise(med_R0 = median(R0),
              ...
              label = first(labels)) %>% 
    filter(round(med_R0, 2) >= round(base_r0_median, 2) | med_R0 <= round(base_r0_median, 2)) %>% 
...
gg_forest <- gg_forest_df %>%
    ggplot(aes(y = as.factor(order), 
      ...
      geom_rect(xmin = base_r0_q25, xmax = base_r0_q75,
                ymin = -Inf, ymax = Inf, 
                alpha = 0.1, fill = "grey80", col = "grey80") +
...
![Forestplot of agrochemical effects with median effect outside of baseline $R_0$ estimates at peak EEC](../Plots/Forestplot_sig_effects_eec.jpg)
...
```{r forestplot_peak_conc, message = FALSE, echo = FALSE, cache=TRUE, warning = FALSE}
#Create labels field for complex axis labels of forestplot
  max_study_string <- max(str_length(RFxSum$study_long))
  max_chem_string <- max(str_length(RFxSum$Chemical))
  max_species_string <- max(str_length(RFxSum$Species))
    ...
    ggplot(aes(y = as.factor(order), 
      ...
      scale_color_manual(name = "Parameter affected",
                         ...
                         labels = c(expression(Snail~reproduction~(italic(f[N]))),
                                    ...
                                    expression(Snail~cercarial~shedding~(italic(theta))),
                                    expression(Schistosome~egg~viability~(italic(v))))) +
      scale_y_discrete(breaks = as.factor(gg_forest_peak_conc_df$order), 
                       labels = gg_forest_peak_conc_df$label) +
      facet_grid(Class~., scales = "free", space = "free") +
      labs(x = expression(paste(italic(R[0]), 
                                "(", italic(POC[c]), ")", 
                                sep = "")))

ggsave("../Plots/Forestplot_sig_effects_peak_conc.jpg",
       plot = gg_forest_peak_conc,
       width = 11.5, height = 8, units = "in", dpi = 300)

```

![Forestplot of agrochemical effects with median effect outside of IQR of baseline $R_0$ estimates at peak observed concentration](../Plots/Forestplot_sig_effects_peak_conc.jpg)

```{r all_peak_obs_forestplot, warning = FALSE}
gg_forest_peak_conc_df_all <-  peak_conc_sims %>% 
    group_by(Study_ID_Unique) %>% 
    summarise(med_R0 = median(R0),
              R0_25 = quantile(R0, 0.25),
              R0_75 = quantile(R0, 0.75),
...
gg_forest_peak_conc_all <- gg_forest_peak_conc_df_all %>%
    ggplot(aes(y = as.factor(order), 
               x = med_R0, xmin = R0_25, xmax = R0_75, 
               col = Parameter)) + 
      theme_classic() +
      ...
      scale_color_manual(name = "Parameter affected",
                         ...
                         labels = c(expression(Snail~reproduction~(italic(f[N]))),
                                    ...
                                    expression(Snail~cercarial~shedding~(italic(theta))),
                                    expression(Schistosome~egg~viability~(italic(v))))) +
      scale_y_discrete(breaks = as.factor(gg_forest_peak_conc_df_all$order), 
                       labels = gg_forest_peak_conc_df_all$label) +
      facet_grid(Class~., scales = "free", space = "free") +
      labs(x = expression(paste(italic(R[0]), 
                                "(", italic(POC[c]), ")", 
                                sep = "")))

ggsave("../Plots/Forestplot_all_effects_peak_conc.jpg",
       plot = gg_forest_peak_conc_all,
       width = 12, height = 15, units = "in", dpi = 300)

```


## Summary agrochemical effects at eecs  
...
sum_eec_effects %>% 
  knitr::kable(row.names = T,
               col.names = c("25th percentile",
                             "median",
                             "75th percentile"),
               caption = "Summary estimates of agrochemical effects on R0 for each chemical")
```
...
obs_boxplot_all <- all_obs_full_chems %>% 
  ...
  ggplot(aes(x = Chemical, y = conc_rel_peak, fill = Chemical)) +
    geom_boxplot(varwidth = TRUE) +
    scale_y_continuous(trans = "log", 
                       breaks = c(0.00001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1),
                       limits = c(0.00001,1.1)) +
    coord_flip() +
    scale_fill_manual(values = c("Atrazine" = "#FDAF91FF",
                                 "Chlorpyrifos" = "#ED000099",
                                 "Glyphosate" = "#42B54099",
                                 "Malathion" = "#925E9F99",
                                 "Profenofos" = "#0099B499")) +
    ylab(expression(paste("Agrochemical Concentration (", italic(q[c]),
                          ") Normalized to ", italic(POC[c])))) +
    ggtitle("Distribution of key agrochemicals observed across all observed datasets",
            subtitle = "Normalized to peak observed concentration") +
    theme_classic()
...
#### Normalized to peak EEC  
```{r boxplot_all_obs_eec_fig}
# Boxplot of observed agrochemical values in MSQA database
obs_boxplot_all_eec <- all_obs_full_chems %>% 
  mutate(conc_rel_peak = case_when(Chemical == "Atrazine" ~ Concentration / atr_eec,
                                   Chemical == "Chlorpyrifos" ~ Concentration / chlor_eec,
                                   Chemical == "Glyphosate" ~ Concentration / gly_eec,
                                   Chemical == "Malathion" ~ Concentration / mal_eec,
                                   Chemical == "Profenofos" ~ Concentration / prof_eec)) %>% 
  ggplot(aes(x = Chemical, y = conc_rel_peak, fill = Chemical)) +
    geom_boxplot(varwidth = TRUE) +
    scale_y_continuous(trans = "log", 
                       breaks = c(0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 2),
                       limits = c(0.000001, 2),
                       labels = c("0.0001", "0.001", "0.01", "0.1", "1", "10", "100", "200")) +
    coord_flip() +
    scale_fill_manual(values = c("Atrazine" = "#FDAF91FF",
                                 "Chlorpyrifos" = "#ED000099",
                                 "Glyphosate" = "#42B54099",
                                 "Malathion" = "#925E9F99",
                                 "Profenofos" = "#0099B499")) +
    ylab(expression(paste("Agrochemical Concentration (", q[c],
                          ") Normalized to ", POC[c]))) +
    ggtitle("Distribution of key agrochemicals observed across all observed datasets",
            subtitle = "Normalized to peak EEC") +
    theme_classic()
...
```{r gen_range_par_sets, cache = T}
source("../Sims/Range/generate_conc_range_parameter_sets.R")
```
  ...
  #Put them all together and plot  
    range_plot <- bind_rows(atr_conc_range_fin,
      ...
      ggplot(aes(x = EEC_conc, 
        ...
        labs(y = expression(paste(R[0], "(", q[c], ")")),
    ...
    ggsave("../Plots/range_plot_2xEEC.jpg", range_plot,
  ...
  #Put them all together and plot  
    range_plot_peak_obs <- bind_rows(atr_peak_obs_conc_range_fin,
      ...
      ggplot(aes(x = rel_conc, 
        ...
        geom_ribbon(alpha = 0.25, size = 0.01) +
        geom_hline(yintercept = base_r0_median, lty = 2) +
        labs(y = expression(italic(R[0](q[c]))),
             x = expression(Agrochemical~Concentration~(italic(q[c]))~Normalized~to~italic(POC[c]))) + 
        #ggtitle(expression(paste("Combined effects of key agrochemicals on estimates of ", R[0], "(q)"))) +
        scale_x_continuous(trans = "log",
                           breaks = c(0.00001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1),
                           labels = c("0.00001", "0.0001", ".001", "0.01", "0.1", "1"),
                           limits = c(0.00001,1.1)) +
        scale_color_manual(values = c("Atrazine" = "#FDAF91FF",
                                      "Chlorpyrifos" = "#ED000099",
                                      "Glyphosate" = "#42B54099",
    ...
    ggsave("../Plots/r0_range_plot_POC.jpg", range_plot_peak_obs,
           width = 8, height = 6, dpi = 300, units = "in")
```
...
range_plot <- ggplotGrob(range_plot + 
                           theme(axis.title.x = element_blank(),
                                 ...
                                 legend.background = element_rect(fill="grey90", 
                                                                  ...
                                                                  size=0.5, 
                                                                  linetype="solid")))
range_box <- ggplotGrob(obs_boxplot_all_eec + 
                          theme(axis.title.y = element_blank(),
...
![Effect of agrochemicals with sufficient information on $R_0$ estimates over a range of feasible environmental concentrations and distribution of observed agrochemical concentrations from all observed datasets](../Plots/r0_range_plot.jpg)
...
#save the plot
jpeg("../Plots/r0_range_plot_obs_conc.jpg", 
     height = 6, width = 8.5, units = "in", res = 300)
...
rm(list = setdiff(ls(), c("fit_pars",
                          "atr_mw2013_ts", "atr_peak_obs",
                          "chlor_ca_surf_ts", "chlor_peak_obs",
                          "gly_ca_surf_ts", "gly_peak_obs",
                          "mal_ca_surf_ts", "mal_peak_obs")))
...
#Generate annual forcing function  
  gen_annual_force_fx <- function(agrochem_ts, norm_start = FALSE){
    if(norm_start){ #Option to normalize starting date of agrochemical time series within the year
...
agrochem_time_series <- data.frame(time = t_sim,
           atrazine = sapply(t_sim, atr_annual_force) / atr_peak_obs,
           glyphosate = sapply(t_sim, gly_annual_force) / gly_peak_obs,
           malathion = sapply(t_sim, mal_annual_force) / mal_peak_obs,
           chlorpyrifos = sapply(t_sim, chlor_annual_force) / chlor_peak_obs) %>% 
  gather("Chem", "Concentration", atrazine: chlorpyrifos)

agrochem_time_series %>% 
  ggplot(aes(x = time, y = Concentration, col = Chem)) +
    geom_line() +
    theme_classic() +
    labs(x = "time (days)",
         y = "Agrochemical concentration normalized to POC",
         title = "Agrochemical time series used in 10-year simulations")
```
...
rm(list = setdiff(ls(), c("fit_pars", "agrochem_time_series",
                          "atr_mw2013_ts", "atr_annual_force",
                          "atr_annual_simulation", #"atr_biannual_simulation",
                          "chlor_ca_surf_ts", "chlor_annual_force",
                          "chlor_annual_simulation", #"chlor_biannual_simulation",
                          "gly_ca_surf_ts", "gly_annual_force",
                          "gly_annual_simulation", #"gly_biannual_simulation",
                          "mal_ca_surf_ts", "mal_annual_force", #"mal_biannual_simulation",
                          "mal_annual_simulation")))
...
comp_dalys_atr_annual <- comp_dalys_across_sims(atr_annual_simulation) %>% 
  mutate(Agro_attributable = Agro_only - Nothing,
      ...
      #Attributable DALYs per 1000 people for comparison to other conditions   
         Agro_att_p1000 = (Agro_attributable/fit_pars["H"])*1000,
         Pred_att_p1000 = (Pred_attributable/fit_pars["H"])*1000,
         MDA_att_p1000 = (MDA_attributable/fit_pars["H"])*1000,
         MDA_Preds_att_p1000 = (MDA_Preds_attributable/fit_pars["H"])/1000,
         Chemical = "Atrazine",
         Application = "Annual")
...
added_dalys <- 
  comp_dalys_all_annual %>% filter(Application == "Annual") %>% 
  mutate(agro_only_effect = Agro_only - Nothing,
         agro_mda_effect = Agro_MDA - MDA_only,
         agro_pred_effect = Agro_Preds - Preds_only,
         agro_mda_pred_effect = Agro_Preds_MDA - MDA_Preds) %>% 
  gather("Agro_effect", "Added_DALYs", agro_only_effect:agro_mda_pred_effect) %>%
  group_by(Chemical) %>% 
...
atr_conc_ts <- data.frame(day = unique(atr_annual_ts$time),
                          atrazine = sapply(atr_annual_ts$time, atr_annual_force))
...
#grid.arrange(atr_dynamics_plot, atr_dalys_plot, 
...
ggsave("../Plots/atrazine_ts_plots2.jpg", atr_grob,
       width = 8, height = 6, dpi = 300, units = "in")
```
...
chlor_conc_ts <- data.frame(day = unique(chlor_annual_ts$time),
                            chlorpyrifos = sapply(chlor_annual_ts$time, chlor_annual_force))

chlor_annual_ts_P <- chlor_annual_ts %>% 
  filter(Variable == "P" & 
           sim_run == 1 & 
           sim_type %in% c("Nothing", "Agro_only", 
                           "MDA_only", "Agro_MDA",
  ...
  ggplot() +
    geom_rect(data = chlor_conc_ts,
              aes(ymin = -Inf,
                  ...
                  fill = chlorpyrifos),
              alpha = 0.03) +
    scale_fill_gradient(low = "white", high = "#ED000099") +
    geom_line(data = chlor_annual_ts_P, 
              aes(x = time, 
                  y = Value, 
                  col = Scenario), 
              size = 1) +
    scale_color_manual(values = c("red", "darkred",
                                  "green", "darkgreen",
                                  "violet", "purple")) +
    labs(y = expression(Predator~Population~(italic(P))),
         x = "Time (years)") +
    scale_x_continuous(breaks = 365*c(0:11),
                       labels = c(-1:10)) +
...
ggsave("../Plots/chlorpyrifos_ts_plots2.jpg", chlor_grob,
       width = 8, height = 6, dpi = 300, units = "in")
```
