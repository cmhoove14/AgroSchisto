...
r0.mal.fix = data.frame(mal = c(cerc.auc.mal$mal,
                        ...
                        par = c(rep('piC', length(cerc.auc.mal$mal)), 
                                ...
                                rep('muN', length(sn$conc_ppb)),
                                rep('fN', length(eg$conc_ppb))),
                        study = c(rep('Tch92', length(cerc.auc.mal$mal)), 
                                  rep('Tch91a', length(mir.auc.mal$mal)),
                                  rep('Halstead15', length(mal.sum$mal.c)),
                                  rep('Bakry11', length(mun.mal$conc)),
                                  rep('Tch91b', length(sn$conc_ppb)*2)),
                        r0 = c(r0.fix(pi_Cqx = cerc.auc.mal$piC[1])[3],
                               ...
                               r0.fix(mu_Nqx = sn$prop_dead[1])[3],
                               r0.fix(mu_Nqx = sn$prop_dead[2])[3],
                               r0.fix(mu_Nqx = sn$prop_dead[3])[3],
                               r0.fix(mu_Nqx = sn$prop_dead[4])[3],
                               r0.fix(mu_Nqx = sn$prop_dead[5])[3],

                               r0.fix(f_Nqx = eg$prop_surv[1])[3],
                               r0.fix(f_Nqx = eg$prop_surv[2])[3],
                               r0.fix(f_Nqx = eg$prop_surv[3])[3],
                               r0.fix(f_Nqx = eg$prop_surv[4])[3],
                               r0.fix(f_Nqx = eg$prop_surv[5])[3]))
...
set.seed(0)
  ...
  #individual parameters
    ...
    r0.fill.mal[, i, 3] = parSapply(clus1, conc.mal, r0.In, f.f_Nq = fN.mal.fx.uncertainty)[3,]
    ...
    r0.fill.mal[, i, 6] = parSapply(clus1, conc.mal, r0.In, f.mu_Nq = muNq_mal_tch91_uncertainty)[3,]
    r0.fill.mal[, i, 7] = parSapply(clus1, conc.mal, r0.In, f.f_Nq = fNq_mal_tch91_uncertainty)[3,]
  ...
  #parameter pairs and triplets
    ...
    #both snail effects (mortality and reproduction) from Bakry
    r0.fill.mal[, i, 9] = parSapply(clus1, conc.mal, r0.In,
                                    f.mu_Nq = muNq_mal_Bakry11_uncertainty,
                                    f.f_Nq = fN.mal.fx.uncertainty)[3,]
    #both snail effects (mortality and reproduction) from Tchounwou91
    r0.fill.mal[, i, 10] = parSapply(clus1, conc.mal, r0.In,
                                    f.mu_Nq = muNq_mal_tch91_uncertainty,
                                    f.f_Nq = fNq_mal_tch91_uncertainty)[3,]
    ...
    r0.fill.mal[, i, 12] = parSapply(clus1, conc.mal, r0.In,
                                    f.mu_Nq = muNq_mal_tch91_uncertainty,
                                    f.f_Nq = fNq_mal_tch91_uncertainty,
                                    f.mu_Pq = muPq_mal_Halstead_uncertainty)[3,]
    #Full simulation w/all parameters
    r0.fill.mal[, i, 13] = parSapply(clus1, conc.mal, r0.In,
                                    f.mu_Nq = muNq_mal_tch91_uncertainty,
                                    f.f_Nq = fNq_mal_tch91_uncertainty,
                                    f.pi_Mq = piM.tch91_mal_unc,
                                    f.pi_Cq = piC.tch92_mal_unc,
  ...
  #fill parameter values
    ...
    par.fill.mal[, i, 3] = parSapply(clus1, conc.mal, fN.mal.fx.uncertainty)
    ...
    par.fill.mal[, i, 6] = parSapply(clus1, conc.mal, muNq_mal_tch91_uncertainty)
    par.fill.mal[, i, 7] = parSapply(clus1, conc.mal, fNq_mal_tch91_uncertainty)
  ...
  #fill equilibrium snail pop estimates  
    ...
    Neq.fill.mal[, i, 3] = parSapply(clus1, conc.mal, r0.In, f.f_Nq = fN.mal.fx.uncertainty)[1,]
    ...
    Neq.fill.mal[, i, 6] = parSapply(clus1, conc.mal, r0.In, f.mu_Nq = muNq_mal_tch91_uncertainty)[1,]
    Neq.fill.mal[, i, 7] = parSapply(clus1, conc.mal, r0.In, f.f_Nq = fNq_mal_tch91_uncertainty)[1,]
    ...
    Neq.fill.mal[, i, 9] = parSapply(clus1, conc.mal, r0.In,
                                    f.mu_Nq = muNq_mal_Bakry11_uncertainty,
                                    f.f_Nq = fN.mal.fx.uncertainty)[1,]
    #both snail effects (mortality and reproduction) from Tchounwou91
    Neq.fill.mal[, i, 10] = parSapply(clus1, conc.mal, r0.In,
                                     f.mu_Nq = muNq_mal_tch91_uncertainty,
                                     f.f_Nq = fNq_mal_tch91_uncertainty)[1,]
    #larval mortality with predator mortality
    ...
    #snail effects with predator mortality
    Neq.fill.mal[, i, 12] = parSapply(clus1, conc.mal, r0.In,
                                     f.mu_Nq = muNq_mal_tch91_uncertainty,
                                     f.f_Nq = fNq_mal_tch91_uncertainty,
                                     f.mu_Pq = muPq_mal_Halstead_uncertainty)[1,]
    #Full simulation w/all parameters
    Neq.fill.mal[, i, 13] = parSapply(clus1, conc.mal, r0.In,
                                     f.mu_Nq = muNq_mal_tch91_uncertainty,
                                     f.f_Nq = fNq_mal_tch91_uncertainty,
                                     f.pi_Mq = piM.tch91_mal_unc,
                                     f.pi_Cq = piC.tch92_mal_unc,
...
conc.mal.means.par[,1] = conc.mal
  for(i in 1:length(parfx)){
    conc.mal.means.par[,(i+1)] = rowMeans(par.fill.mal[ , , i])
  }
