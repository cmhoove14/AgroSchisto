...
#fit models to L.2 parameters across concentration ########
  ...
  bl.but = lm(b ~ but, weights = b.se^-1, data = cerc.but)   
    ...
    lines(seq(0,7000,7), sapply(seq(0,7000,7), bl.pred)[1,], lty = 2, col = 2)
    lines(seq(0,7000,7), sapply(seq(0,7000,7), bl.pred)[2,], lty = 3, col = 2)
    lines(seq(0,7000,7), sapply(seq(0,7000,7), bl.pred)[3,], lty = 3, col = 2)
...
#plot sample output compared to observed AUCs ##########
  ...
  keep.tant.but.pic = c('piC.tant02_but.lin_unc', 'piC.tant02_but.exp_unc', 'bl.but',
                        'el.but2', 'el.but', 'L.3.fx', 'tant.but.pic.aucs')      
