...
ons = read.csv('~/RemaisWork/Schisto/Data/AgroData/Data/Snail Mortality/Omran&Salama2013.csv')
  ...
  ons$surv = 1 - ons$mort
  ons$ppmlog10 = log10(ons$ppm+1)
  
ons.atr = subset(ons, chem == 'atrazine')
ons.gly = subset(ons, chem == 'glyphosate')

#Atrazine mortality ############
plot(ons.atr$ppmlog10, ons.atr$probit, pch = 16, col = 'gold', #ylim = c(0,7), xlim = c(0, 3),
     xlab = 'atrazine (ppm)', ylab = c('Probit mortality')) 
  lm.ons.atr = lm(probit ~ ppmlog10, data = ons.atr)
...
fx.ons.atr = function(He){
  heu = log10(He/1000)
  pnorm(predict(lm.ons.atr, newdata = data.frame(ppmlog10 = heu), 
...
#glyphosate toxicity ############   
plot(ons.gly$ppmlog10, ons.gly$probit, pch = 16, #xlim = c(0,3), ylim = c(4,6)
     col = 3)  
  lm.ons.gly = lm(probit ~ ppmlog10, data = ons.gly)  
...
fx.ons.gly = function(He){
  heu = log10((He/1000)+1)
  pnorm(predict(lm.ons.gly, newdata = data.frame(ppmlog10 = heu), 
          interval = 'confidence', level = 0.95), mean = 5)
} 
...
ons.munq.gly = function(He){
  heu = log10((He/1000)+1)
  init = predict(lm.ons.gly, newdata = data.frame(ppmlog10 = heu), se.fit = T)
  est = rnorm(1, init$fit, init$se.fit)
    while(est < 0){
      est = rnorm(1, init$fit, init$se.fit)
    }
