...
mod_comm_2018_2016 <- mod_comm %>% group_by(School) %>% summarise(diff = )
...
mod_comm_prawn <- mod_comm_veg <- mod_comm_ref <- mod_comm
  mod_comm_ref <- mod_comm_ref %>% mutate(Prawns = 0,
                                          Veg = 0,
                                          Net = 0)
  mod_comm_veg <- mod_comm_ref %>% mutate(Veg = ifelse(year == 2018, 1, 0))
  mod_comm_prawn <- mod_comm_ref %>% mutate(Prawns = ifelse(year == 2018, 1, 0),
                                            Net = ifelse(year == 2018, 1, 0))

comm_ref_out <- predict(mod1_haem_comm_all, newdata = mod_comm_ref)

comm_prawn_out <- predict(mod1_haem_comm_all, newdata = mod_comm_prawn)

comm_veg_out <- predict(mod1_haem_comm_all, newdata = mod_comm_veg)

mod1_counter <- data.frame(cbind(School = mod_comm$School, year = mod_comm$year, 
                                 comm_ref_out, comm_prawn_out, comm_veg_out)) %>% 
  gather(key = "intervention", value = "Prediction", comm_ref_out:comm_veg_out) %>% 
  mutate(Prediction = as.numeric(Prediction))

mod1_counter %>% filter(intervention != "comm_veg_out" & year == 2018) %>% 
  ggplot(aes(x = intervention, y = Prediction, col = School, group = School)) +  
    geom_point(stat='summary', fun.y=sum) +
    stat_summary(fun.y=sum, geom="line") 

#That's basically just showing the point estimate of the prawn coefficient across all villages which doesn't actually tell us much.
```
...
#Package for prediction with GEE models
require(doBy)

gee_predict_prawn <- gee_predict_veg <- gee_predict_ref <- full_long

gee_predict_prawn <- gee_predict_prawn %>%
  mutate(Prawns = ifelse(year_fac==2018,1,0),
         Net = ifelse(year_fac==2018,1,0))

gee_prawn_prediction <- prediction(gee_ex, newdata = gee_predict_prawn)
```
