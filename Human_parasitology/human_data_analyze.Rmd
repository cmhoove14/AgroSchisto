...
```{r comm_mod1}
#Model 1
mod1_haem_comm_all <- glm.nb(round(haem_mean) ~ Net + Veg + Prawns + School + pzq, 
                             weights = haem_samps,
                             data = mod_comm)
...
mod00_haem_comm_all <- glm.nb(round(haem_mean) ~ year_fac + School + extra_pzq, 
  ...
  mod00_haem_comm_all_res <- robust_fx(mod00_haem_comm_all)
    mod00_haem_comm_all_res$model <- "Model 0"
...
#Unlist to rebind with the full df
  ...
  mod1_perm <- glm.nb(round(haem_mean) ~ Net + Veg + Prawns + School + pzq, 
                      weights = haem_samps,
                      data = mod_perm)
  ...
  return(c(mod1_int_coef, mod2_int_coef))
}
...
colnames(perm_result) <- c("Mod1_Veg", "Mod1_Net", "Mod1_Prawns",
...
gee_ex <- gee(s_haem_bin_narm ~ Net + Veg + Prawns + School + pzq + sex, id = full_long$Child_ID,
...
res_gee_ex = summary(gee_ex)$coefficients
  ...
  colnames(gee_ex_sum) = c('OR', 'OR.025', 'OR.975', 'Naive SE', 'Robust SE')

as.data.frame(gee_ex_sum) %>%     
  mutate(coefficient = rownames(gee_ex_sum)) %>% 
  ggplot(aes(x = OR, y = coefficient)) + theme_bw() +
    geom_point() + 
    geom_errorbarh(aes(xmin = OR.025, xmax = OR.975, y = coefficient), height = 0.25) +
